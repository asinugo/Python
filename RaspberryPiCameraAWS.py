...
# Create string with the OS Command 
		remote_ip = "3.86.29.188" # Variable to hold the Ip address of the remote host becuase it is dynamic and changes all the time
		#string to contatenate the parts of the command 
		string = filename + " " + "ubuntu@"+ remote_ip+":~/motion/" +filename
		cmd = "scp %s" % string
		#os module to execute the scp command as if it was typed 
		os.system(cmd)
		#command to remove files from the system 
		cmd = "rm -f %s" % filename
		os.system(cmd)
